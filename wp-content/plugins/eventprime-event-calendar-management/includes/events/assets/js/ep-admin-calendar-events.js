jQuery( function( $ ) {
    ...
    function initilizeEventCalendar( cal_initial_date, events ){
        ...
        var utcOffsetMinutes = currentDate.getTimezoneOffset();
        var utcOffsetSign = utcOffsetMinutes !== 0 ? (utcOffsetMinutes <= 0 ? '+' : '-') : '';
        var utcHours = utcOffsetSign + Math.abs(Math.floor(utcOffsetMinutes / 60));
            ...
            calendar = new FullCalendar.Calendar( calendarEl, {
                ...
                titleFormat: function (info) {
                    if ( info.date.timeZoneOffset < 0 ) {       // -ve UTC 
                        var start = formatDate(new Date(info.start.marker.getTime() + 86400000), em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format); 
                        var end = formatDate(new Date(info.end.marker.getTime()), em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format);
                        ...
                        var end; 
                        if ( ( utcHours === '+1' && info.date.month == 9 ) ) {
                            end = formatDate(info.end.marker, em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format);
                        } else {
                            end = formatDate(new Date(info.end.marker.getTime() - 86400000), em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format);
                        }
                    } else {
                        var start = formatDate(info.start.marker, em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format); 
                        var end; 
                        if ( utcHours === '+1' && info.date.month == 2 ) {
                            end = formatDate(new Date(info.end.marker.getTime() - 43200000), em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format);
                        } else {
                            end = formatDate(info.end.marker, em_admin_calendar_event_object.local, eventprime.global_settings.calendar_title_format);
                        }
                    }
