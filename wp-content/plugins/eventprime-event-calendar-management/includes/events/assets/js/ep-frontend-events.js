                ...
                titleFormat: function (info) {
                    ...
                    if ( info.date.timeZoneOffset < 0 ) {       // -ve UTC 
                        ...
                        if ( ( utcHours === '+1' && info.date.month == 9 ) ) {
                            end = formatDate(info.end.marker, em_front_event_object.local, eventprime.global_settings.calendar_title_format);
                        } else {
                            end = formatDate(new Date(info.end.marker.getTime() - 86400000), em_front_event_object.local, eventprime.global_settings.calendar_title_format);
                        }
                    } else {
                        var start = formatDate(info.start.marker, em_front_event_object.local, eventprime.global_settings.calendar_title_format); 
                        var end; 
                        if ( utcHours === '+1' && info.date.month == 2 ) {
                            end = formatDate(new Date(info.end.marker.getTime() - 43200000), em_front_event_object.local, eventprime.global_settings.calendar_title_format);
                        } else {
                            end = formatDate(info.end.marker, em_front_event_object.local, eventprime.global_settings.calendar_title_format);
                        }
                    }
