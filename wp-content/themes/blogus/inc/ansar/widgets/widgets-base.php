 ...
 * Base Widget Class
 */
class Blogus_Widget_Base extends WP_Widget
{
    /**
     * @var Array of string
     */
     ...
     */
    public function blogus_generate_text_input($field, $label, $value, $type = 'text', $note = '', $class = '')
    {
     ...
     * @param string $note field note to appear below
     */
    public function blogus_generate_textarea($field, $label, $note = '')
    {
     ...
     */
    public function blogus_generate_checkbox_input($field, $label, $elements, $note = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : true;
        ?>
        <div class="blogus-multiple-check-form">
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($field)); ?>">
                    <?php echo esc_html($label); ?>
                </label>
     ...
     * @param Array_A $elements
     */
    public function blogus_generate_select_options($field, $label, $elements, $note = '')
    {
    ...
    public function blogus_generate_image_upload($field, $label, $value, $note = '', $class = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : '';
        ?>
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="button" class="select-img button button-primary"
                       value="<?php esc_attr_e('Upload', 'blogus'); ?>"
                       data-uploader_title="<?php esc_attr_e('Select Image', 'blogus'); ?>"
                       data-uploader_button_text="<?php esc_attr_e('Choose Image', 'blogus'); ?>"/>
                <?php
                $image_status = false;
                if (!empty($instance)) {
                    $image_status = true;
                ...
                ?>
                <input type="button" value="<?php echo esc_attr('X', 'Remove', 'blogus'); ?>"
    ...
    {
        $instance = array();
        $instance = $this->blogus_sanitize_data($instance, $new_instance);
        return $instance;
    }

    public function blogus_sanitize_data($instance, $new_instance)
    {
        if (is_array($this->text_fields)) {
            // update the text fields values
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->blogus_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->blogus_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->blogus_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->blogus_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->blogus_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
     ...
     */
    public function blogus_update_text($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function blogus_update_url($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function blogus_update_textarea($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function blogus_update_checkbox($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function blogus_update_select($name, $new_instance)
    {
        $instance = array();
